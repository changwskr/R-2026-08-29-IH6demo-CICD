...
1. jenkins-node1 Docker 서버 실행
    ...
    (jenkins-node1 로그인) 하여 jdk와 git을 설치한다.
...
(jenkins-server로 로그인) jenkins-server에서 node1로 자유롭게 로그인 가능하도록 작업한다.
#-----------------------------------------------------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    sh-4.4# mkdir slave1
    sh-4.4# ls
    Dockerfile  anaconda-ks.cfg  anaconda-post.log  original-ks.cfg  slave1
    sh-4.4# cd slave1
    sh-4.4# pwd
    /root/slave1
    sh-4.4#
